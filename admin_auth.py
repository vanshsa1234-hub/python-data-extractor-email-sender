...
def register_user(username: str, password: str) -> tuple[bool, str]:
    """
    Register a new user account.
    If username+password match ADMIN_USERNAME+ADMIN_PASSWORD in secrets,
    the account is silently granted admin role.
    Returns (success, message).
    """
    username = username.strip().lower()
    ...
    # Check if this matches the secret admin credentials
    secret_admin_user, secret_admin_pass = _get_admin_credentials()
    if secret_admin_user and username == secret_admin_user and password == secret_admin_pass:
        role = "admin"
    else:
        role = "user"
