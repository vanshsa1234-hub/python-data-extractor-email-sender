...
def _get_admin_credentials() -> tuple[str, str]:
    """Read admin username+password from Streamlit secrets or environment."""
    admin_username = ""
    admin_password = ""
    try:
    ...
    return admin_username.strip().lower(), admin_password.strip()
...
def register_user(username: str, password: str) -> tuple[bool, str]:
    ...
    They are created automatically on first login using
    ...
    secret_admin_user, _ = _get_admin_credentials()

    if secret_admin_user and username == secret_admin_user:
...
def login_user(username: str, password: str) -> tuple[bool, dict]:
    ...
    - Normal users authenticate from the database.
    - Admin authenticates using Streamlit Secrets.
    - First admin login automatically creates the admin account.
    """
    ...
    # Secret Admin Login
    # -------------------------------------------------------
    secret_admin_user, secret_admin_pass = _get_admin_credentials()

    if (
        secret_admin_user
        and username == secret_admin_user
        and password == secret_admin_pass
    ):
...
def save_user_leads(user_id: int, leads: list[dict]) -> int:
    ...
    for lead in leads:
        ...
        try:
            exists = conn.execute(
                "SELECT 1 FROM user_leads WHERE user_id=? AND email=?",
                (user_id, email)
            ).fetchone()
            if not exists:
                conn.execute(
            ...
            log.warning("Error saving lead %s: %s", email, e)
    conn.commit()
    conn.close()
    log.info("save_user_leads: %d new leads saved for user_id=%d", inserted, user_id)
    return inserted
